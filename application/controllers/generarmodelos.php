...
class Generarmodelos extends CI_Controller {
    ...
    function __construct() {
        ...
        private $copyright = "IDATI LTDA.";
        private $email = "ofuenzalida@idati.cl";
...
EOT;
    ...
    private function _export() {
        ...
        foreach ($this->models as $name => $data) {
            $fullpath = FCPATH . APPPATH . 'models/' . $name . '_m' . EXT;
    ...
    private function _format_output() {
        ...
        foreach ($this->tables as $name => $fields) {
            ...
            $str = sprintf($this->header, ucfirst($name), $this->copyright, $this->author, $this->email, date("d-m-Y H:i:s"), strtolower($name), $fields[0]["name"]);
            $validate = "";
...
//                $str .= ', ' . $field['default'] . "),\n";
            ...
            $str .= sprintf($this->footer, strtolower($name), $this->body_model);
